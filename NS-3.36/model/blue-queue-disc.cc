...
void
BlueQueueDisc::SetMode (QueueSizeUnit mode)
{
...
BlueQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  if ((GetMode () == QueueSizeUnit::PACKETS && nQueued >= m_queueLimit)
      || (GetMode () == QueueSizeUnit::BYTES && nQueued + item->GetPacket()->GetSize () > m_queueLimit))
    {
      // Increment the Pmark
      IncrementPmark ();
...
void BlueQueueDisc::DecrementPmark (void)
{
  ...
  if (m_isIdle)
    {
      double m = 0.0; // stores the number of times Pmark should be decremented
      m = ((now - m_idleStartTime) / m_freezeTime).GetDouble();
